package Core;


public class PropertyKeys {
    public static String SAVED_DATA_PATH = "savedData/";

    public static String DELIMITER_KEY = "DELIMITER";
    public static String BOT_TOKEN_KEY = "BOT_KEY";
    public static String SAVED_DATA_PATH_KEY = "SAVED_DATA_PATH";
    public static String MONITORED_REACTION_KEY = "MONITORED_REACTION";
    public static String ROLE_ASSIGNMENT_CHANNEL_KEY = "ROLE_ASSIGNMENT_CHANNEL";
    public static String AUTO_ROLES_KEY = "AUTO_ROLES";
    public static String SERVER_NAME_KEY = "SERVER_NAME";

    public static String JDBC_USER_KEY = "JDBC_USER";
    public static String JDBC_PASS_KEY = "JDBC_PASS";
    public static String JDBC_DB_NAME_KEY = "JDBC_DB_NAME";

    public static String IPC_CLASS_PATH_KEY = "IPC_CLASS_PATH";
}
